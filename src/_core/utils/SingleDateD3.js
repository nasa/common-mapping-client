...
export default class SingleDateD3 {
    ...
    initValues(options) {
        ...
        this._symbolWidthLarge =
            typeof options.symbolWidthLarge !== "undefined"
                ? options.symbolWidthLarge
                : this._symbolWidthLarge;
        ...
        this._beforeDrag =
            typeof options.beforeDrag !== "undefined" ? options.beforeDrag : this._beforeDrag;
        this._onDrag = typeof options.onDrag !== "undefined" ? options.onDrag : this._onDrag;
        this._afterDrag =
            typeof options.afterDrag !== "undefined" ? options.afterDrag : this._afterDrag;
        this._getNearestDate =
            typeof options.getNearestDate !== "undefined"
                ? options.getNearestDate
                : this._getNearestDate;
        this._getDateFromX =
            typeof options.getDateFromX !== "undefined" ? options.getDateFromX : this._getDateFromX;
        this._getXFromDate =
            typeof options.getXFromDate !== "undefined" ? options.getXFromDate : this._getXFromDate;
    ...
    updatePosition(transition = false) {
        ...
        if (
            !datum.isDragging &&
            this._getXFromDate(datum.date) !== parseFloat(this._selection.attr("x"))
        ) {
            if (transition) {
                this._selection
                    .transition()
    ...
    prepDrag() {
        let drag = d3.behavior
            ...
            .on("drag", () => {
                ...
                // check for scroll
                if (x >= maxX) {
                    this._selection.attr("x", d => maxX).attr("transform", d => {
                        return "translate(" + maxX + ",0)";
                    });
                    scrollFlag = 1;
                } else if (x <= minX) {
                    this._selection.attr("x", d => minX).attr("transform", d => {
                        return "translate(" + minX + ",0)";
                    });
                    scrollFlag = -1;
                } else {
                    this._selection.attr("x", d => x).attr("transform", d => {
