...
function timeScaleD3(linear, methods, format) {
    ...
    scale.nice = function(interval, skip) {
        ...
        let method = interval == null ? tickMethod(extent, 10) : typeof interval === "number" && tickMethod(extent, interval);
        ...
        return scale.domain(scaleNice(domain, skip > 1 ? {
            floor: function(date) {
                while (skipped(date = interval.floor(date))) date = scaleDate(date - 1);
                return date;
            },
            ceil: function(date) {
                while (skipped(date = interval.ceil(date))) date = scaleDate(+date + 1);
                return date;
            }
        } : interval));
    };
    ...
    scale.ticks = function(interval, skip) {
        ...
        let method = interval == null ? tickMethod(extent, 10) : typeof interval === "number" ? tickMethod(extent, interval) : !interval.range && [{ range: interval }, skip]; // assume deprecated range function
